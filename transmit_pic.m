...
mm = load('pic_data.mat');
binary_data = de2bi(mm.B,8);
...
%         fprintf('block err_ = %d \n ',err);
    ...
    decoded = reshape(binary_pb(1:length(sig))',[], 8);
    receive_pic = uint8(bi2de(decoded));
    receive_pic = reshape(receive_pic,648,464);
    figure;
    imshow(receive_pic);
end
